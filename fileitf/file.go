...
type FileReqItf interface {
	...
	CreateFile(hash string, size int64) (fileCode int64, err string)
	//OpenFile @path is file's virtual(or net) path.
	OpenFile(hash string) (fileCode, size int64, err string)
	//Read read from fileCode.
	Read(fileCode, block int64) (accessCode int32, err string)
	//Write write to fileCode.
	Write(fileCode, block int64) (accessCode int32, err string)
